...
// Negro a rojo
for (i = 0; i != 256; i++) {
  fieldColors.push(1, 0, 0); // De negro (0,0,0) a rojo (1,0,0)
}
...
function main() {
  ...
  const hiddenOptions = ["conducting planes w/ gap", "slotted conducting plane", "fast charge", "conducting plane + pt", "charged line", "line charge double", "dipole lines", "quad lines", "finite line", "finite line pair", "finite line dipole", "conducting plate", "charged plate", "charged plate pair", "infinite plane", "conducting sphere + pt", "conducting sphere in field", "charged ring", "charged ring pair", "charged ring dipole", "dielectric sphere in field, E", "dielectric sphere in field, D", "dielectric cyl in field, E", "dielectric cyl in field, D", "dielectric boundary E", "dielectric boundary D", "charged sphere + pt","cylinder + line charge", "cylinder in field"];
  
  while (f != null) {
    if (!hiddenOptions.includes(f.getName())) { // Ocultar opciones definidas en hiddenOptions
...
function drawScene(gl, buffers, deltaTime) {
  ...
  gl.clearColor(1.0, 1.0, 1.0, 1.0); // Cambiado a blanco
...
// Ajuste para las flechas de los ejes (color negro)
function drawAxes(gl, buffers, viewMatrix) {
  ...
  // Escalar para normalizar los ejes
  var scale = 4 * 0.4 / zoom3d;
  mat4.scale(scaledViewMatrix, viewMatrix, [scale, scale, scale, 1]);

  // Deshabilitar el test de profundidad
  gl.disable(gl.DEPTH_TEST);

  // Guardar y restaurar zoom
  var saveZoom = zoom3d;
  zoom3d = 0.4;

  // Asegurar el uso del programa de shaders correcto
  gl.useProgram(colorPlainProgramInfo.program);

  // Configurar color negro para las flechas
  const blackColor = [0.0, 0.0, 0.0, 1.0]; // RGBA
  gl.uniform4fv(colorPlainProgramInfo.uniformLocations.color, blackColor);

  // Dibujar las flechas de los ejes
  drawArrow(gl, buffers, projectionMatrix, scaledViewMatrix, [0, 0, 0], [1, 0, 0], [1, 1, 1], false, inverseMatrix); // Eje X
  drawArrow(gl, buffers, projectionMatrix, scaledViewMatrix, [0, 0, 0], [0, 1, 0], [1, 1, 1], false, inverseMatrix); // Eje Y
  drawArrow(gl, buffers, projectionMatrix, scaledViewMatrix, [0, 0, 0], [0, 0, 1], [1, 1, 1], false, inverseMatrix); // Eje Z

  // Restaurar el zoom original
  zoom3d = saveZoom;

  // Configurar las etiquetas para los ejes
  var labels = "xyz";
  for (var i = 0; i < 3; i++) {
      var vec = vec4.create();
      vec[i] = 1.3; vec[3] = 1;
  ...
  // Restaurar la matriz de proyección principal
...
function drawSphere(position, scale, color) {
  ...
  if (color == undefined)
    ...
    gl.uniform4f(programInfo.uniformLocations.color, 0.1, 0.6, 1.0, 1);
...
function drawCube(gl, buffers, projectionMatrix, viewMatrix) {
  ...
  for (var i = 0; i != 6; i++) {
    ...
    gl.uniformMatrix4fv(colorPlainProgramInfo.uniformLocations.projectionMatrix, false, projectionMatrix);
    gl.uniformMatrix4fv(colorPlainProgramInfo.uniformLocations.modelViewMatrix,  false, modelViewMatrix);
    gl.uniform4f(colorPlainProgramInfo.uniformLocations.color, 0, 0, 0, 1);
    gl.drawArrays(gl.LINE_LOOP, 0, verts.length/3);
...
class InverseSquaredRadial extends VecFunction {
  getName() { return BUILD_CASE_EMV("Carga Puntual", null, "1/r^2 single"); }
  getFieldMult() { return .0003/coulombKQ; }
  getPotMult() { return .1/coulombKQ; }
  getInfo() { return "Carga = " + (-reverse) + " nC<br>"; }
  getField(result, y) {
    var r = distance(y);
    if (r < chargeSize)
      boundCheck = true;
...
class InverseSquaredRadialDouble extends InverseSquaredRadial {
  getName() { return BUILD_CASE_EMV("Carga Puntual Doble", null, "1/r^2 double"); }
  getBestSlice() { return SLICE_Y; }
  getInfo() {
     var sep = auxBarValue(1)/100.;
     return "Cargas = " + (this.sign2 == 1 ? -reverse : "&plusmn;1") + " nC<br>Separación = " + round(sep*2) + " m<br>";
  }
  getField(result, y) {
    var sep = auxBarValue(1)/100.;
    var xx1 = y[0]-sep;
  ...
  setup() {
    setXZView();
    this.sign2 = 1;
    setupBar(0, "Separación entre cargas", 30);
  }
  ...
  createNext() { return BUILD_CASE_EMV(new InverseSquaredRadialDipole(), null, new InverseRadial()); }
	...
	getName() { return "Cuadripolo (2 Dipolos)"; }
        getInfo() {
          var sep = auxBarValue(1)/100.;
          return "Cargas = " + (this.sign2 == 1 ? "1" : "&plusmn;1") + " nC<br>Separacion = " + round(sep*2) + " m<br>";
        }
	getField(result, y) {
	    var sep = auxBarValue(1)/100.;
	    var xx1 = y[0] - sep;
